 ...
 */
{
  "hitLog": {
    ...
    "formatHeader": "<textformat leading='6'><font face='$FieldFont' size='15' color='#F2F2F2'>{{l10n:Hits}}: <b><font color='#FFFFFF'>{{hitlog.n}}</font></b>  {{l10n:Total}}: <b><font color='#FFCC66'>{{hitlog.dmg-total}}</font></b>  {{l10n:Last}}: <b><font color='#FFFFFF'>{{dmg}}</font></b></font></textformat>",
    // List of hits format (macros allowed, see macros.txt).
    // Формат лога попаданий (допускаются макроподстановки, см. macros.txt).
    "formatHistory": "<textformat leading='-2' tabstops='[22,96,129,167,213]'><font face='$FieldFont' size='13' color='{{hitlog.dead?#FFE1A6|#F2F2F2}}'>&#xD7;{{hitlog.n-player}}<tab>{{dmg-kind}}<tab>-{{dmg}}<tab>({{hitlog.dmg-player}})<tab><font face='XVMSymbol' size='19'>{{hitlog.dead|&#x2A;}}</font><tab><font face='XVMSymbol' size='20'>{{vtype}}</font>  {{vehicle}}</font></textformat>"
  }
}
