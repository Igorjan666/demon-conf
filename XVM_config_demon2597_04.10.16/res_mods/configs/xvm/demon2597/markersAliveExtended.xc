  ...
  "def": {
     ...
     "textFields": [
            ...
            // Ник игрока
            {
              ...
              "format": "{{name%.12s~..}} <font size='14' color='{{topclan?#FFCC66|#E5D39A}}'>{{clannb}}</font>"  //  формат текста. См. описание макросов в macros.txt
            },
            // Рейтинг и количество боев
            {
              ...
              "format": "<font size='{{xvm-stat?13|0}}'><font color='{{c:t-battles}}'>{{t-hb>=99.5?XXh|{{t-hb%d~h|--h}}}}</font>  <font color='{{c:xte}}'>{{xte%s|--}}</font>  <font color='{{c:r}}'>{{r_size=2?{{r%s|--}}|{{r>=10000?XXXX|{{r%d|----}}}}}}</font>  <font color='{{c:kb}}'>{{kb>=99.5?XXk|{{kb%d~k|--k}}}}</font></font>"  //  формат текста. См. описание макросов в macros.txt
            },
